    ...
    isLoading(false)
{
    ...
    ui->labelSubtitle2->setText(tr("Accept BTCA delegation / Delegate BTCA"));
    ...
    ui->btnCoinControl->setSubTitleClassAndText("text-subtitle", "Select BTCA outputs to delegate.");
...
void ColdStakingWidget::updateStakingTotalLabel()
{
    ...
    ui->labelStakingTotal->setText(tr("Total Staking: %1").arg(
            (total == 0) ? "0.00 BTCA" : GUIUtil::formatBalance(total, nDisplayUnit))
    );
}
