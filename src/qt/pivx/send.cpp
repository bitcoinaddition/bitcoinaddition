    ...
    btnContacts(new QPushButton())
{
    ...
    /* Subtitle */
    ui->labelSubtitle1->setText(tr("You can transfer BTCA"));
    setCssProperty(ui->labelSubtitle1, "text-subtitle");

    /* Address */
    ui->labelSubtitleAddress->setText(tr("BTCA address or contact label"));
    ...
    ui->labelAmountSend->setText("0.00 BTCA");
...
void SendWidget::refreshView()
{
    ...
    ui->pushButtonSave->setText(tr("Send BTCA"));
...
void SendWidget::showHideCheckBoxDelegations()
{
    ...
    // coincontrol is not selected, and we are trying to spend BTCA (not zBTCA)
...
void SendWidget::onCoinControlClicked()
{
    if (walletModel->getBalance() > 0) {
        ...
        inform(tr("You don't have any BTCA to select."));
    }
}
