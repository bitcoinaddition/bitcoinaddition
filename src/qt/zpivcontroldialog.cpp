    ...
    model(0)
{
    ...
    // Title
    ui->labelTitle->setText(tr("Select zBTCA Denominations to Spend"));
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < nRequiredConfs)
                ...
                strReason = "Your wallet is locked. Impossible to spend zBTCA.";
            else if (!mint.isSeedCorrect)
                strReason = "The zBTCA seed used to mint this zBTCA is not the same as currently hold in the wallet";
            else
