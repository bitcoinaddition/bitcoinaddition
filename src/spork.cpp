...
std::vector<CSporkDef> sporkDefs = {
    ...
    MAKE_SPORK_DEF(SPORK_5_MAX_VALUE,                       1000),          // 1000 BTCA
    MAKE_SPORK_DEF(SPORK_8_MASTERNODE_PAYMENT_ENFORCEMENT,  1609576230), // OFF
    MAKE_SPORK_DEF(SPORK_9_MASTERNODE_BUDGET_ENFORCEMENT,   1609576230), // OFF
    MAKE_SPORK_DEF(SPORK_13_ENABLE_SUPERBLOCKS,             1609576230), // OFF
	...
	MAKE_SPORK_DEF(SPORK_21_COLDSTAKING_ENFORCEMENT,        1609576230), // OFF
	MAKE_SPORK_DEF(SPORK_22_BAN_DUPLICATE_MN_PER_IP,        1609576230), // OFF
...
void CSporkManager::ProcessSpork(CNode* pfrom, std::string& strCommand, CDataStream& vRecv)
{
    ...
    if (strCommand == "spork") {
        ...
        // reject old signatures 50 blocks after hard-fork
        if (spork.nMessVersion != MessageVersion::MESS_VER_HASH) {
            if (Params().GetConsensus().IsMessSigV2(nChainHeight - 50)) {
