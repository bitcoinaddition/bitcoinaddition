...
{
    {
        ...
        if (zwalletMain)
        {
            ...
            if (CWalletDB(strWalletFile).ReadCurrentSeedHash(hashSeed))
            {
                ...
                {
                    return error("Failed to read zBTCA seed from DB. Wallet is probably corrupt.");
                }
...
) const
{
...

    {
        ...
        for (std::map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it)
        {
            ...
            if (nCoinType == STAKEABLE_COINS && nDepth < Params().GetConsensus().nStakeMinDepth)
                continue;
            ...
            for (unsigned int i = 0; i < pcoin->vout.size(); i++)
            {
                ...
                if (nCoinType == STAKEABLE_COINS && pcoin->vout[i].IsZerocoinMint())
                    continue;
                if (IsSpent(wtxid, i))
                    continue;
                ...
                bool fIsValid = (((mine & ISMINE_SPENDABLE) != ISMINE_NO) ||
...
bool CWallet::GetBudgetSystemCollateralTX(CWalletTx &tx, uint256 hash, bool useIX)
{
    ...
    vecSend.push_back(std::make_pair(scriptChange, BUDGET_FEE_TX_OLD)); // Old 50 BTCA collateral
...
bool CWallet::GetBudgetFinalizationCollateralTX(CWalletTx &tx, uint256 hash, bool useIX)
{
    ...
    std::string strFail = "";
    std::vector<std::pair<CScript, CAmount>> vecSend;
    vecSend.push_back(std::make_pair(scriptChange, BUDGET_FEE_TX)); // New 5 BTCA collateral
        ...
        LOCK2(cs_main, cs_wallet);
        {
            ...
            while (true)
            {
                ...
                if (nChange > 0)
                {
                    ...
                    // change transaction isn't always pay-to-btca-address
                    CScript scriptChange;
...
bool CWallet::CreateCoinStakeKernel(CScript &kernelScript, const CScript &stakeScript, unsigned int nBits, const CBlock &blockFrom, const CTransaction &txPrev, const COutPoint &prevout, unsigned int &nTimeTx, bool fPrintProofOfStake) const
{
    ...
    for (unsigned int i = 0; i < nHashDrift; ++i)
    {
        ...
        if (CheckStakeKernelHash(nBits, blockFrom, txPrev, prevout, nTryTime, nHashDrift, true, hashProofOfStake))
        {
            ...
            if (chainActive.Tip()->nHeight > 650000 && nTryTime > GetAdjustedTime() + Params().GetConsensus().nFutureTimeDriftPoS)
            {
...
{
    const CChainParams::Base58Type addrType =
        AddressBook::IsColdStakingPurpose(purpose) ? CChainParams::STAKING_ADDRESS : CChainParams::PUBKEY_ADDRESS;
