        ...
        std::list<std::pair<CBitcoinAddress*,CAmount>> addressesTo, CBitcoinAddress* changeAddress)
{
    ...
    receipt.SetStatus("Spend Successful", ZPIV_SPEND_OKAY);  // When we reach this point spending zBTCA was successful
        ...
        CBitcoinAddress* changeAddress)
{
    ...
    if (nValue < 1) {
        receipt.SetStatus(_("Value is below the smallest available denomination (= 1) of zBTCA"), nStatus);
        return false;
    }
    ...
    const int nMaxSpends = Params().GetConsensus().ZC_MaxPublicSpendsPerTx; // Maximum possible spends for one zBTCA public spend transaction
    std::vector<CMintMeta> vMintsToFetch;
    if (vSelectedMints.empty()) {
        //  All of the zBTCA used in the public coin spend are mature by default (everything is public now.. no need to wait for any accumulation)
        ...
        // Select the zBTCA mints to use in this spend
...
{
    if (fMatureOnly) {
        // This code is not removed just for when we back to use zBTCA in the future, for now it's useless,
