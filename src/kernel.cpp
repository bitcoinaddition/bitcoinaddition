...
{
    // BTCA will hash in the transaction hash and the index number in order to make sure each hash is unique
...
bool CheckProofOfStake(const CBlock block, uint256 &hashProofOfStake)
{
    ...
    if (!CheckStakeKernelHash(block.nBits, blockprev, txPrev, txin.prevout, nTime, nInterval, true, hashProofOfStake))
        return error("CheckProofOfStake() : INFO: check kernel failed on coinstake %s, hashProof=%s \n", tx.GetHash().ToString().c_str(), hashProofOfStake.ToString().c_str()); // may occur during initial download or if behind on blockchain sync
